 ...
 */
function buildInputSchema(op: Record<string, unknown>): {
  type: string;
  ...
  if (
    ...
    const bodySchema = (
      op.requestBody.content["application/json"] as { schema: unknown }
    ).schema;
...
function buildOutputSchema(op: Record<string, unknown>): unknown {
  if (op.responses && typeof op.responses === "object") {
    ...
    const resp = (responses["200"] ?? responses["201"]) as
...
export function extractMcpToolsFromOpenApi(
  ...
  for (const [pathKey, pathItemRaw] of Object.entries(
