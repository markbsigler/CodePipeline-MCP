...
export function jsonSchemaToZod(schema: unknown): ZodTypeAny {
  ...
  if (Array.isArray(s.enum) && s.enum.length > 0) {
    ...
    if (enumArr.every((v) => typeof v === 'string')) {
      ...
      if (enumArr.length >= 2) {
        ...
        return z.union(enumArr.map((v) => z.literal(v)) as unknown as [ZodTypeAny, ZodTypeAny, ...ZodTypeAny[]]);
  ...
  if ('const' in s && s.const !== undefined) {
    ...
    if (typeof constVal === 'string' || typeof constVal === 'number' || typeof constVal === 'boolean' || constVal === null) {
  ...
  // Handle oneOf/anyOf
  const arr = (Array.isArray(s.oneOf) && s.oneOf.length > 0)
    ? s.oneOf
    : (Array.isArray(s.anyOf) && s.anyOf.length > 0 ? s.anyOf : undefined);
  if (arr) {
    if (arr.length >= 2) {
      return z.union(arr.map((v) => jsonSchemaToZod(v)) as [ZodTypeAny, ZodTypeAny, ...ZodTypeAny[]]);
  ...
  if (type === 'string') {
    let str = z.string();
    if ('minLength' in s && typeof s.minLength === 'number') {str = str.min(s.minLength);}
    if ('maxLength' in s && typeof s.maxLength === 'number') {str = str.max(s.maxLength);}
    if ('pattern' in s && typeof s.pattern === 'string') {str = str.regex(new RegExp(s.pattern));}
    base = str;
  } else if (type === 'number' || type === 'integer') {
    let num = z.number();
    if ('minimum' in s && typeof s.minimum === 'number') {num = num.min(s.minimum);}
    if ('maximum' in s && typeof s.maximum === 'number') {num = num.max(s.maximum);}
    ...
    if (!('properties' in s) || typeof s.properties !== 'object' || s.properties === null) {return z.record(z.any());}
