...
describe('validateBody middleware', () => {
  ...
  it('calls next() and attaches parsed data for valid body', () => {
  ...
  it('returns 400 and error details for invalid body', () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ error: 'Invalid request body', details: expect.any(Array) })
    );
    expect(next).not.toHaveBeenCalled();
  });

  it('works with schemas that allow extra fields', () => {
