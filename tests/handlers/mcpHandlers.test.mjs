...
describe('mcpHandlers', () => {
    ...
    describe('toolsCallHandler', () => {
        ...
        beforeAll(() => {
            toolZodSchemas['echo'] = {
                input: { safeParse: (params) => ({ success: params && params.ok, data: params, error: { errors: ['fail'] } }) }
            };
        });
        ...
        it('returns 404 if tool not found', async () => {
            ...
            await toolsCallHandler(mcpTools, openapi)({}, res);
        ...
        it('streams all branches and cleans up stream state', async () => {
            ...
            const res = mockRes();
            res.req.body = { tool: 'echo', params: { ok: true } };
            const handlerPromise = toolsCallHandler(mcpTools, openapi)({}, res);
            ...
            expect(writeCalls.some((c) => c.includes('partialResult'))).toBe(true);
    ...
    describe('notificationsListChangedHandler', () => {
        it('returns changed: false', () => {
            const res = mockRes();
            notificationsListChangedHandler()({}, res);
